...
def create_final_trade_decider(llm):
    ...
    def trade_decision_node(state) -> dict:
        ...
        volume_report = state.get('volume_report', 'No volume analysis available.') # [NEW]
        ...
        prompt = f"""You are a high-frequency quantitative trading (HFT) analyst operating on the current {time_frame} K-line chart for {stock_name}. Your task is to issue an **immediate execution order**: **LONG** or **SHORT**. ⚠️ HOLD is prohibited due to HFT constraints.
            ...
            - **Confirm price moves with volume**:
              - **Rising prices + Rising volume** = Strong bullish signal.
              - **Rising prices + Falling volume** = Weak bullish (divergence), potential reversal.
              - **Falling prices + Rising volume** = Strong bearish signal.
            - **Look for climaxes**: Extreme volume spikes often mark tops or bottoms.
            - **Accumulation/Distribution**: Signs of institutional buying/selling.
            ...
            2. Prioritize decisions where **all four reports** (Indicator, Pattern, Trend, Volume) **align in the same direction**.
            ...
            - **Volume confirmation** (breakouts with volume are more reliable)
            ...
            6. **CRITICAL**: Calculate a **dynamic risk-reward ratio** between **1.2 and 1.8** based on:
               - **Higher ratio (1.6-1.8)**: Strong trend, clear breakout, high conviction signals
               - **Medium ratio (1.4-1.5)**: Moderate signals, some alignment across reports
               - **Lower ratio (1.2-1.3)**: Weak signals, consolidation, or conflicting reports
               - **DO NOT use 1.5 as default** — analyze the actual market conditions!
            ...
            ```
            {{
            "forecast_horizon": "Predicting next 3 candlesticks (15 minutes, 1 hour, etc.)",
            "decision": "<LONG or SHORT>",
            "justification": "<Concise, confirmed reasoning based on reports>",
            "risk_reward_ratio": "<MUST be a float between 1.2 and 1.8, calculated based on signal strength>"
            }}
